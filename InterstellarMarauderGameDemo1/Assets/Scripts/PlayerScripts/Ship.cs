...
public class Ship : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            rocketfire = false;
            foreach (RocketLauncher launcher in rockets)
            {
                if (launcher.gameObject.activeSelf)
                {
                    launcher.ShootRocket();
                }
            }
        }
...
private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        BossDestructable bossDestructable = collision.GetComponent<BossDestructable>();
        if (bossDestructable != null)
        {
            Hit(bossDestructable.gameObject);
        }
        ...
        if (powerUp != null)
        {
            ...
            if (powerUp.giveRocketAmmo && rocketLauncher != null) // add null check for rocketLauncher
            {
                rocketLauncher.AddAmmo(5);
                Debug.Log("Current ammo: " + rocketLauncher.currentAmmo);
            }
